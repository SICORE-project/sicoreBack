...
return new class extends Migration
{
    ...
    {
        // 1. Ajout des colonnes à la table enseignants (avec vérifications)
        Schema::table('enseignants', function (Blueprint $table) {
            // Vérifier si matricule n'existe pas déjà
            if (!Schema::hasColumn('enseignants', 'matricule')) {
                $table->string('matricule')->nullable()->unique()->after('id');
            }
            
            // Vérifier si salaire_base n'existe pas déjà
            if (!Schema::hasColumn('enseignants', 'salaire_base')) {
                $table->decimal('salaire_base', 14, 2)->default(0);
            }
            
            // Vérifier si nombre_parts n'existe pas déjà
            if (!Schema::hasColumn('enseignants', 'nombre_parts')) {
                $table->unsignedTinyInteger('nombre_parts')->default(1);
            }
            
            // Vérifier si actif n'existe pas déjà
            if (!Schema::hasColumn('enseignants', 'actif')) {
                $table->boolean('actif')->default(true);
            }
            
            // Vérifier si numero_compte n'existe pas déjà
            if (!Schema::hasColumn('enseignants', 'numero_compte')) {
                $table->string('numero_compte')->nullable();
            }
            
            // Vérifier si etablissement_id n'existe pas déjà
            if (!Schema::hasColumn('enseignants', 'etablissement_id')) {
                $table->foreignId('etablissement_id')->nullable();
            }
        });

        // 2. Ajouter la contrainte étrangère séparément (si la table existe)
        ...
        // 3. Créer les tables payroll (avec vérifications)
        ...
        if (!Schema::hasTable('payroll_attendances')) {
        ...
        if (!Schema::hasTable('payroll_elements')) {
    ...
    public function down(): void
    {
        ...
        Schema::table('enseignants', function (Blueprint $table) {
            ...
            $columns = ['matricule', 'salaire_base', 'nombre_parts', 'actif', 'numero_compte'];
            foreach ($columns as $column) {
                if (Schema::hasColumn('enseignants', $column)) {
                    // Pour matricule, on doit d'abord supprimer l'index unique
                    if ($column === 'matricule') {
